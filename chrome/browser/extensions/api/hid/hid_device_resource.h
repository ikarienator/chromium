...
class HidDeviceResource : public ApiResource {
 public:
  HidDeviceResource(const std::string& owner_extension_id,
                    scoped_refptr<HidDevice> device);

  virtual ~HidDeviceResource();
 ...
 private:
  friend class ApiResourceManager<HidDeviceResource>;
  ...
  scoped_refptr<HidDevice> device_;

  DISALLOW_COPY_AND_ASSIGN(HidDeviceResource);
};
